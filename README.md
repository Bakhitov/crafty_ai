...
- Web search теперь использует пользовательские ключи (per-user). Добавьте свой EXA ключ через API `POST /api/user/keys` с `provider="exa"`. Legacy переменная `EXA_API_KEY` больше не требуется.
